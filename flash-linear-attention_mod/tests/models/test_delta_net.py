...
import pytest
from fla.models.delta_net import DeltaNetConfig, DeltaNetNoTritonModel
...
def test_deltanet_forward_and_backward_pass():
    ...
    config = DeltaNetConfig(
        ...
        # attn_mode="naive_chunk",
        attn_mode="gla_mod_chunk",
        chunk_size=32,
        sigmoid_scale=2.0
    )
    
    model = DeltaNetNoTritonModel(config)
    model.eval()
    ...
    dummy_target = torch.randn(batch_size, seq_length, config.hidden_size)  # random target with same shape as output
